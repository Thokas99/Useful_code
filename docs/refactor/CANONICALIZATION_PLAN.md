...
| Quarto report guidance | Former Quarto reference; CODE_MAP/quarto_config_tmplate.md | Current guidance is maintained under `miscellaneous/quarto/`; no active cheatsheet collection is retained. | Unique sections from both former guides. | YAML, captions, cross-references, metrics, and handoff guidance. | Duplicate guides and the misspelled filename. | None. | Notebook analysis templates are distinct. | `miscellaneous/quarto/scientific_report_template.md` | High | Compare unique prose before consolidation; no runtime validation needed beyond a small example render later. |
...
The graph below identifies source roles. It is not a filesystem move plan.
    ...
    simple_QC_MGI_organoids
      ...
      <- former RNA-seq normalization reference
...
| Former Quarto reference or CODE_MAP/quarto_config_tmplate.md | One guide could replace the other. | Compare unique sections manually. |
